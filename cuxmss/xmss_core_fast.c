...
// static void treehash_init_parallel(const xmss_params *params,
// 				   unsigned char *node, int height, int index,
// 				   bds_state *state, const unsigned char *sk_seed,
// 				   const unsigned char *pub_seed, const uint32_t addr[8])
// {
// 	printf("parallel treehash init\n");
//
// 	unsigned int idx = index;
// 	// use three different addresses because at this point we use all three formats in parallel
// 	uint32_t ots_addr[8] = { 0 };
...
// 	}
//
// 	unsigned char stack1[height * params->n];
// 	unsigned int stacklevels1[height];
// 	unsigned int stackoffset1 = 0;
//
// 	i = 0;
// 	for (; idx < lastnode / 2; idx++) {
// 		set_ltree_addr(ltree_addr, idx);
// 		set_ots_addr(ots_addr, idx);
// 		gen_leaf_wots(params, stack1 + stackoffset1 * params->n, sk_seed, pub_seed, ltree_addr, ots_addr);
// 		stacklevels1[stackoffset1] = 0;
// 		stackoffset1++;
// 		if (params->tree_height - params->bds_k > 0 && i == 3) {
// 			memcpy(state->treehash[0].node, stack1 + stackoffset1 * params->n, params->n);
// 		}
// 		while (stackoffset1 > 1 && stacklevels1[stackoffset1 - 1] == stacklevels1[stackoffset1 - 2]) {
// 			nodeh = stacklevels1[stackoffset1 - 1];
// 			if (i >> nodeh == 1) {
// 				memcpy(state->auth + nodeh * params->n, stack1 + (stackoffset1 - 1) * params->n, params->n);
// 			}else {
// 				if (nodeh < params->tree_height - params->bds_k && i >> nodeh == 3) {
// 					memcpy(state->treehash[nodeh].node, stack1 + (stackoffset1 - 1) * params->n, params->n);
// 				}else if (nodeh >= params->tree_height - params->bds_k) {
// 					memcpy(state->retain + ((1 << (params->tree_height - 1 - nodeh)) + nodeh - params->tree_height + (((i >> nodeh) - 3) >> 1)) * params->n, stack1 + (stackoffset1 - 1) * params->n, params->n);
// 				}
// 			}
// 			set_tree_height(node_addr, stacklevels1[stackoffset1 - 1]);
// 			set_tree_index(node_addr, (idx >> (stacklevels1[stackoffset1 - 1] + 1)));
// 			thash_h(params, stack1 + (stackoffset1 - 2) * params->n, stack1 + (stackoffset1 - 2) * params->n, pub_seed, node_addr);
// 			stacklevels1[stackoffset1 - 2]++;
// 			stackoffset1--;
// 		}
// 		i++;
// 	}
//
// 	// 不使用原始stack, stacklevels和stackoffset
// 	unsigned char stack2[height * params->n];
// 	unsigned int stacklevels2[height];
// 	unsigned int stackoffset2 = 0;
//
// 	// i应该表示到第几个叶子节点了
// 	for (; idx < lastnode; idx++) {
// 		set_ltree_addr(ltree_addr, idx);
// 		set_ots_addr(ots_addr, idx);
// 		gen_leaf_wots(params, stack2 + stackoffset2 * params->n, sk_seed, pub_seed, ltree_addr, ots_addr);
// 		stacklevels2[stackoffset2] = 0;
// 		stackoffset2++;
// 		if (params->tree_height - params->bds_k > 0 && i == 3) {
// 			memcpy(state->treehash[0].node, stack2 + stackoffset2 * params->n, params->n);
// 		}
// 		while (stackoffset2 > 1 && stacklevels2[stackoffset2 - 1] == stacklevels2[stackoffset2 - 2]) {
// 			nodeh = stacklevels2[stackoffset2 - 1];
// 			if (i >> nodeh == 1) {
// 				memcpy(state->auth + nodeh * params->n, stack2 + (stackoffset2 - 1) * params->n, params->n);
// 			}else {
// 				if (nodeh < params->tree_height - params->bds_k && i >> nodeh == 3) {
// 					memcpy(state->treehash[nodeh].node, stack2 + (stackoffset2 - 1) * params->n, params->n);
// 				}else if (nodeh >= params->tree_height - params->bds_k) {
// 					memcpy(state->retain + ((1 << (params->tree_height - 1 - nodeh)) + nodeh - params->tree_height + (((i >> nodeh) - 3) >> 1)) * params->n, stack2 + (stackoffset2 - 1) * params->n, params->n);
// 				}
// 			}
// 			set_tree_height(node_addr, stacklevels2[stackoffset2 - 1]);
// 			set_tree_index(node_addr, (idx >> (stacklevels2[stackoffset2 - 1] + 1)));
// 			thash_h(params, stack2 + (stackoffset2 - 2) * params->n, stack2 + (stackoffset2 - 2) * params->n, pub_seed, node_addr);
// 			stacklevels2[stackoffset2 - 2]++;
// 			stackoffset2--;
// 		}
// 		i++;
// 	}
...
// 	memcpy(state->auth + nodeh * params->n, stack2 + (stackoffset2 - 1) * params->n, params->n);
// 	set_tree_height(node_addr, stacklevels2[stackoffset2 - 1]);
// 	set_tree_index(node_addr, 0);
// 	memcpy(stack1 + params->n, stack2, params->n);
// 	thash_h(params, stack1, stack1, pub_seed, node_addr);
// 	stacklevels2[stackoffset2 - 2]++;
// 	stackoffset2--;
//
// 	for (i = 0; i < params->n; i++) {
		      ...
		      unsigned char *pk, unsigned char *sk)
{
	...
	for (unsigned int iter = 0; iter < 2 * params->n; iter++) {
		sk[params->index_bytes + iter] = 2;
	}
	// randombytes(sk + params->index_bytes, 2 * params->n);

	// Init PUB_SEED (n byte)
	for (unsigned int iter = 0; iter < params->n; iter++) {
		sk[params->index_bytes + 3 * params->n + iter] = 4;
	}
	...
	// treehash_init_parallel(params, pk, params->tree_height, 0, &state, sk + params->index_bytes, sk + params->index_bytes + 3 * params->n, addr);
	// copy root to sk
